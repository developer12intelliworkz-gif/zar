'use client';

import { useCallback, useState } from 'react';
import Image from 'next/image';
import { Controller, useForm } from 'react-hook-form';
import Button from '@/components/ui/atoms/Button/Button';
import CustomCaptcha from '@/components/ui/molecules/CustomCaptcha/CustomCaptcha';
import InputField from '@/components/ui/atoms/InputField/InputField';
import PhoneField from '@/components/ui/atoms/PhoneField/PhoneField';
import TextareaField from '@/components/ui/atoms/TextareaField/TextareaField';
import { useToast } from '@/components/ui/Toast/ToastContext';
import { useAppSelector } from '@/store/hooks';
import { cn, getImageUrl } from '@/lib/utils';
import { submitProductInquiry } from '@/lib/api/contact';
import styles from './CartEnquiryModal.module.css';

interface CartEnquiryModalProps {
  isOpen: boolean;
  onClose: () => void;
}

type EnquiryFormValues = {
  name: string;
  email: string;
  phone: string;
  companyName: string;
  message: string;
};

const NAME_REGEX = /^[A-Za-z][A-Za-z\s'.-]{1,79}$/;
const COMPANY_REGEX = /^[A-Za-z0-9][A-Za-z0-9\s'&.,()-]{1,99}$/;
const EMAIL_REGEX = /^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$/;
const MESSAGE_REGEX = /^[A-Za-z0-9\s.,'"?!@#$%&*()\-:+;[\]{}]{10,1000}$/;

export default function CartEnquiryModal({ isOpen, onClose }: CartEnquiryModalProps) {
  const { showToast } = useToast();
  const { items } = useAppSelector((state) => state.cart);
  
  const [captchaValue, setCaptchaValue] = useState('');
  const [isCaptchaValid, setIsCaptchaValid] = useState(false);
  const [captchaRefreshKey, setCaptchaRefreshKey] = useState(0);

  const handleCaptchaStatusChange = useCallback(
    ({ value, isValid }: { value: string; isValid: boolean }) => {
      setCaptchaValue(value);
      setIsCaptchaValid(isValid);
    },
    []
  );

  const {
    register,
    control,
    handleSubmit,
    reset,
    formState: { errors, isSubmitting },
  } = useForm<EnquiryFormValues>({
    defaultValues: {
      name: '',
      email: '',
      phone: '',
      companyName: '',
      message: '',
    },
    mode: 'onBlur',
  });

  const onSubmit = handleSubmit(async (values) => {
    if (!captchaValue || !isCaptchaValid) {
      showToast('Please complete the 4-digit captcha correctly before submitting.', 'error');
      return;
    }

    try {
      // Extract phone metadata from data attributes
      const phoneInput = document.getElementById('cart-modal-phone') as HTMLInputElement | null;
      const dialCode = phoneInput?.getAttribute('data-dial-code') || '';
      const countryIso = phoneInput?.getAttribute('data-country-code') || '';
      
      const cleanDialCode = dialCode.replace(/\D/g, '');
      const cleanFullNumber = values.phone.replace(/\D/g, '');
      let mobile_no = cleanFullNumber;
      if (cleanDialCode && cleanFullNumber.startsWith(cleanDialCode)) {
        mobile_no = cleanFullNumber.slice(cleanDialCode.length);
      }

      const payload = {
        fullName: values.name,
        companyName: values.companyName,
        email: values.email,
        contactNumber: values.phone,
        products: items.map((item) => item.name),
        message: values.message,
      };

      console.log('Product inquiry API request payload:', payload);

      const response = await submitProductInquiry(payload);
      console.log('Product inquiry API response:', response);

      if (!response.success) {
        showToast(response.message || 'Unable to submit your enquiry. Please try again.', 'error');
        return;
      }

      showToast('Enquiry details captured successfully!', 'success');
      
      // Reset form and close modal
      reset();
      setCaptchaValue('');
      setIsCaptchaValid(false);
      setCaptchaRefreshKey((current) => current + 1);
      onClose();
    } catch (err) {
      console.error('Error submitting cart enquiry:', err);
      showToast('Network error. Please try again in a moment.', 'error');
    }
  });

  const handleOverlayClick = (e: React.MouseEvent<HTMLDivElement>) => {
    if (e.target === e.currentTarget) {
      onClose();
    }
  };

  return (
    <div
      className={cn(styles.overlay, isOpen && styles.overlayOpen)}
      onClick={handleOverlayClick}
      aria-hidden="true"
    >
      <div className={styles.modal} role="dialog" aria-modal="true">
        {/* Close Button */}
        <button
          className={styles.closeBtn}
          onClick={onClose}
          aria-label="Close modal"
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M18 6L6 18M6 6L18 18" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </button>

        {/* Left Column: Form details */}
        <div className={styles.leftPanel}>
          <h2 className={styles.heading}>Product Enquiry</h2>
          <p className={styles.subheading}>Please enter your details to complete your cart product enquiry.</p>

          <form className={styles.form} onSubmit={onSubmit} noValidate>
            <div className={styles.formRow}>
              <InputField
                id="cart-modal-name"
                label="Full Name"
                placeholder="Your full name"
                wrapperClassName={styles.inputGroup}
                required
                errorMessage={errors.name?.message}
                {...register('name', {
                  required: 'Full name is required.',
                  pattern: {
                    value: NAME_REGEX,
                    message: 'Enter a valid full name.',
                  },
                  minLength: {
                    value: 2,
                    message: 'Full name must be at least 2 characters.',
                  },
                  maxLength: {
                    value: 80,
                    message: 'Full name cannot exceed 80 characters.',
                  },
                })}
              />


               <InputField
                id="cart-modal-company"
                label="Company Name"
                placeholder="Enter company name"
                wrapperClassName={styles.inputGroup}
                errorMessage={errors.companyName?.message}
                {...register('companyName', {
                  validate: (value) =>
                    !value || COMPANY_REGEX.test(value) || 'Enter a valid company name.',
                  maxLength: {
                    value: 100,
                    message: 'Company name cannot exceed 100 characters.',
                  },
                })}
              />
            </div>

            <div className={styles.formRow}>
            

           
              <InputField
                id="cart-modal-email"
                type="email"
                label="Email ID"
                placeholder="you@email.com"
                wrapperClassName={styles.inputGroup}
                required
                errorMessage={errors.email?.message}
                {...register('email', {
                  required: 'Email is required.',
                  pattern: {
                    value: EMAIL_REGEX,
                    message: 'Enter a valid email address.',
                  },
                })}
              />

                <Controller
                name="phone"
                control={control}
                rules={{
                  required: 'Contact number is required.',
                  validate: (value) => {
                    const digits = value.replace(/\D/g, '');
                    return (
                      (digits.length >= 7 && digits.length <= 15) ||
                      'Enter a valid contact number.'
                    );
                  },
                }}
                render={({ field }) => (
                  <PhoneField
                    id="cart-modal-phone"
                    name={field.name}
                    label="Contact No."
                    placeholder="Enter number"
                    wrapperClassName={styles.inputGroup}
                    required
                    value={field.value}
                    onChange={field.onChange}
                    onBlur={field.onBlur}
                    errorMessage={errors.phone?.message}
                  />
                )}
              />
            </div>

            <div className={styles.formRow}>
              <TextareaField
                id="cart-modal-message"
                label="Message"
                placeholder="Your message..."
                wrapperClassName={styles.inputGroup}
                errorMessage={errors.message?.message}
                {...register('message', {
                  validate: (value) => {
                    if (!value) {
                      return true;
                    }
                    return MESSAGE_REGEX.test(value) || 'Message contains invalid characters.';
                  },
                  minLength: {
                    value: 10,
                    message: 'Message must be at least 10 characters.',
                  },
                  maxLength: {
                    value: 1000,
                    message: 'Message cannot exceed 1000 characters.',
                  },
                })}
              />
            </div>

            <CustomCaptcha key={captchaRefreshKey} onStatusChange={handleCaptchaStatusChange} />

            <Button variant="primary" showArrow type="submit" disabled={isSubmitting}>
              {isSubmitting ? (
                <span className={styles.submitLoadingInline}>
                  <span>Submitting...</span>
                </span>
              ) : (
                'Submit Enquiry'
              )}
            </Button>
          </form>
        </div>

        {/* Right Column: Selected items list */}
        <div className={styles.rightPanel}>
          <h3 className={styles.panelTitle}>Your Selection</h3>
          
          <div className={styles.itemList}>
            {items.map((item) => (
              <div key={item.id} className={styles.item}>
                <div className={styles.itemImage}>
                  <Image
                    src={getImageUrl(item.image)}
                    alt={item.name}
                    fill
                    style={{ objectFit: 'cover' }}
                    sizes="60px"
                  />
                </div>
                <div className={styles.itemDetails}>
                  <h4 className={styles.itemName}>{item.name}</h4>
                  {item.purity && (
                    <p className={styles.itemPurity}>
                      Purity: {item.purity === '22k' ? '22K Gold' : item.purity === '18k' ? '18K Gold' : item.purity}
                    </p>
                  )}
                  <p className={styles.itemQty}>Qty: {item.quantity}</p>
                </div>
              </div>
            ))}
          </div>

          <div className={styles.summaryFooter}>
            <div className={styles.totalRow}>
              <span>Total Items</span>
              <span className={styles.totalVal}>
                {items.reduce((acc, item) => acc + item.quantity, 0)}
              </span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
